...
import {useCreate} from '../create/use-create'
...
const copyValue = (item: ConnectionData) => {
    let httpParams = [];
    item.httpParams.forEach((keyValueMap) => {
        httpParams.push({
            key: keyValueMap.prop,
    ...
    return {
        ...
        httpContentType: item.httpContentType,
        requestDataFormat: item.requestDataFormat,
        httpParams: httpParams,
        httpBody: item.httpBody,
        httpCheckCondition: item.httpCheckCondition,
...
const props = {
    item: {
        type: Object as PropType<ConnectionData>,
        require: true
    }
}

export default defineComponent({
    name: 'ResourceConnectionEdit',
    props,
    setup: function (props, ctx) {
        ...
        state.fileForm = copyValue(props.item)

        watch(
            () => props.item,
            () => {
                state.fileForm = copyValue(props.item)
                state.httpBodyResponse = undefined
            }
        )
    ...
    render() {
        const {t} = useI18n()
        return (
            <Card title={t('resource.connection.edit_connection')}>
                <NForm
                    rules={this.rules}
