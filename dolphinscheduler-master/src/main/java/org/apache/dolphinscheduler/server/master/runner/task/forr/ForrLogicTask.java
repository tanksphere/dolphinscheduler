...
@Slf4j
public class ForrLogicTask extends BaseAsyncLogicTask<ForrParameters> {

    public static final String TASK_TYPE = "FORR";
    ...
    public ForrLogicTask(TaskExecutionContext taskExecutionContext,
        ...
        super(taskExecutionContext,
                JSONUtils.parseObject(taskExecutionContext.getTaskParams(), new TypeReference<ForrParameters>() {
                }));
...
//        }
        if (parameterGroup.size() > taskParameters.getMaxNumOfSubWorkflowInstances()) {
            log.warn("getAsyncTaskExecuteFunction error: the number of sub process instances [{}] exceeds the maximum limit [{}]", parameterGroup.size(),
                    taskParameters.getMaxNumOfSubWorkflowInstances());
            throw new MasterTaskExecuteException("the number of sub process instances exceeds the maximum limit");
        }
        ...
        return new ForrAsyncTaskExecuteFunction(taskExecutionContext, processInstance, taskInstance, this,
                commandMapper,
    ...
    public void generateSubWorkflowInstance(List<Map<String, String>> parameterGroup) throws MasterTaskExecuteException {
        ...
        for (Map<String, String> parameters : parameterGroup) {
            String forrStartParams = JSONUtils.toJsonString(parameters);
            Command command = ForrCommandUtils.createCommand(processInstance, subProcessDefinition.getCode(),
            ...
            // somethings
            command.setId(-1);
            ForrCommandUtils.addDataToCommandParam(command, CommandKeyConstants.CMD_FORR_START_PARAMS,
                    forrStartParams);
    ...
    public List<Map<String, String>> generateParameterGroup() {
        List<ForrInputParameter> forrInputParameters = getForrInputParameters();
        ...
        List<Map<String, String>> parameterGroup = cartesianProduct.stream().map(
                inputParameterList -> inputParameterList.stream().collect(
                        Collectors.toMap(ForrInputParameter::getName, ForrInputParameter::getValue)))
    ...
    private List<ForrInputParameter> getForrInputParameters() {
        List<ForrInputParameter> forrInputParameters = taskParameters.getListParameters();
        if (CollectionUtils.isNotEmpty(forrInputParameters)) {
            for (ForrInputParameter forrInputParameter : forrInputParameters) {
                String value = forrInputParameter.getValue();
                Map<String, Property> paramsMap = taskExecutionContext.getPrepareParamsMap();
                value = ParameterUtils.convertParameterPlaceholders(value, ParameterUtils.convert(paramsMap));
                forrInputParameter.setValue(value);
            }
        }
        return forrInputParameters;
    }
