...
import static org.apache.dolphinscheduler.common.constants.CommandKeyConstants.CMD_FORR_START_PARAMS;

@Slf4j
public class ForrAsyncTaskExecuteFunction implements AsyncTaskExecuteFunction {
    ...
    private final ForrLogicTask logicTask;

    public ForrAsyncTaskExecuteFunction(TaskExecutionContext taskExecutionContext,
                                        ProcessInstance processInstance,
                                        TaskInstance taskInstance,
                                        ForrLogicTask forrLogicTask,
                                        CommandMapper commandMapper,
                                        SubWorkflowService subWorkflowService,
                                        int degreeOfParallelism) {
        this.processInstance = processInstance;
        this.taskInstance = taskInstance;
        this.logicTask = forrLogicTask;
    ...
    private void setOutputParameters() {
        ...
        List<ForrOutput> forrOutputs = new ArrayList<>();
        int index = 1;
        for (ProcessInstance processInstance : allSubProcessInstance) {
            ForrOutput forrOutput = new ForrOutput();
            Map<String, String> dynamicParams =
                    JSONUtils.toMap(JSONUtils.toMap(processInstance.getCommandParam()).get(CMD_FORR_START_PARAMS));
            forrOutput.setForrParams(dynamicParams);
            ...
            forrOutput.setOutputValue(outputValueMap);
            forrOutput.setMappedTimes(index++);
            forrOutputs.add(forrOutput);
        }
        ...
        property.setValue(JSONUtils.toJsonString(forrOutputs));
    ...
    private void startSubProcessInstances(List<ProcessInstance> allSubProcessInstance, int startCount) {
        ...
        for (int i = 0; i < Math.min(startCount, waitingProcessInstances.size()); i++) {
            ...
            Map<String, String> parameters = JSONUtils.toMap(ForrCommandUtils
                    .getDataFromCommandParam(subProcessInstance.getCommandParam(), CMD_FORR_START_PARAMS));
            Command command = ForrCommandUtils.createCommand(this.processInstance,
    ...
    public List<ProcessInstance> getAllSubProcessInstance() {
        return subWorkflowService.getAllForrSubWorkflow(processInstance.getId(), taskInstance.getTaskCode());
    }
