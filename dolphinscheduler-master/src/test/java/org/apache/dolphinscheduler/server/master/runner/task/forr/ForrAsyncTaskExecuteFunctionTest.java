...
import org.apache.dolphinscheduler.server.master.runner.task.forr.ForrAsyncTaskExecuteFunction;
...
class ForrAsyncTaskExecuteFunctionTest {
    ...
    private ForrAsyncTaskExecuteFunction function;
    ...
    void setUp() {
        ...
        function = new ForrAsyncTaskExecuteFunction(
                null,
                processInstance,
                taskInstance,
                forrLogicTask,
    ...
    void shouldReturnSuccessWhenAllSubProcessInstancesFinishedSuccessfully() {
        ...
        Mockito.when(subWorkflowService.getAllForrSubWorkflow(1, 2L)).thenReturn(processInstances);
        ...
        // When
        ForrParameters forrParameters = new ForrParameters();
        Mockito.when(forrLogicTask.getTaskParameters()).thenReturn(forrParameters);
    ...
    void shouldReturnFailedWhenLogicTaskIsCancelled() {
        ...
        List<ProcessInstance> processInstances = Arrays.asList(Mockito.mock(ProcessInstance.class));
        Mockito.when(subWorkflowService.getAllForrSubWorkflow(1, 2L)).thenReturn(processInstances);
        Mockito.when(forrLogicTask.isCancel()).thenReturn(true);
