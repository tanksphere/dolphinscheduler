...
class ForrCommandUtilsTest {
    ...
    void testCreateCommand() {
        Command command = ForrCommandUtils.createCommand(processInstance, subProcessDefinitionCode,
                subProcessDefinitionVersion, parameters);

        Assertions.assertEquals(CommandType.FORR_GENERATION, command.getCommandType());
    ...
    void testGetDataFromCommandParam() {
        ...
        ForrCommandUtils.addDataToCommandParam(command, "testKey", "testData");
        String data = ForrCommandUtils.getDataFromCommandParam(command.getCommandParam(), "testKey");
    ...
    void testCreateCommandCommandType() {
        ...
        processInstance.setCommandType(CommandType.START_PROCESS);
        Command command1 = ForrCommandUtils.createCommand(processInstance, subProcessDefinitionCode,
                subProcessDefinitionVersion, parameters);
        Assertions.assertEquals(CommandType.FORR_GENERATION, command1.getCommandType());

        // Scenario 2: CommandType is not START_PROCESS
        processInstance.setCommandType(CommandType.START_FAILURE_TASK_PROCESS);
        Command command2 = ForrCommandUtils.createCommand(processInstance, subProcessDefinitionCode,
                subProcessDefinitionVersion, parameters);
    ...
    void testCreateCommandStartParams() {
        ...
        String startParamsJson = ForrCommandUtils.getDataFromCommandParam(command.getCommandParam(),
